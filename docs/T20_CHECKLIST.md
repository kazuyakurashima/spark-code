...
| # | 項目 | 種別 | 結果 |
|---:|---|---|---|
| 1 | ユーザーが Lesson 1〜6 を無料で体験できる | 自動 + 手動 | ✅ 自動 + 🟡 手動確認待ち |
| 2 | 1 周目終了時に、HTML/CSS/JS/Sparkコーチの全体像が見える | 手動 | 🟡 手動確認待ち |
| 3 | Lesson 6 後に自然な課金導線(UI のみ)が出る | 自動 + 手動 | ✅ 自動 + 🟡 手動確認待ち |
| 4 | 各レッスンに、目的・コード入力・プレビュー・Sparkコーチフィードバックがある | 自動 + 手動 | ✅ 自動 + 🟡 手動確認待ち |
| 5 | Sparkコーチの 4 役割(常設ボタン 5 つ)が UI に反映されている | 自動 | ✅ 自動完了 |
| 6 | 画面上部に現在地表示がある | 自動 | ✅ 自動完了 |
| 7 | ユーザーが「自分のカードが育っている」と感じられる | 手動(主観) | 🟡 手動確認必須 |
| 8 | TODO_PHASE3.md が作成され、各タスクに対応要件が紐づいている | 自動 | ✅ 完了(21 タスク × 82 件の対応要件参照) |
| 9 | PHASE3_REQUIREMENTS_CHECK.md が作成され、完了タスクごとの要件適合チェックが記録されている | 自動 | ✅ 完了(T1-T19 + グループ 4 判断済み事項記録済) |
| 10 | Codex Review で APPROVED を得る | T21 で実施 | ⏳ 未着手 |
...
- [ ] **未来のカードプレビュー**: 左に「今のカード」(白背景シンプル)、右に「未来のカード」(紫→ピンクのグラデカード、subtle pulse アニメ)
...
- [ ] DevTools Network タブで `/api/chat` レスポンスに `ANTHROPIC_API_KEY` 値が含まれていない
...
§18.1 10 項目のうち **5 項目(#5、#6、#8、#9、+ #10 は T21 待ち)** は自動確認のみで完了判定可能。

残り 5 項目(#1、#2、#3、#4、#7)は手動確認後に最終判定:
