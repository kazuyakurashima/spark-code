...
- 実装内容:
  - 当時は `BASE_PREVIEW_CSS` を抽出し、Lesson 1 / 2 の previewCss はこれを extend する形に refactor(T22-T27 で最小装飾設計に変更)
...
- 実装内容:
  - 当時の Lesson 4 専用 `LESSON_4_PREVIEW_CSS`: 背景を白系(`#f8fafc`)、`<h1>` の初期 `color: black`、`<p>` `<li>` は slate-600 系。「初期は地味、CSS で色を変えると激変」というコントラストを優先(T22-T27 で Lesson 1-5 共通の最小装飾へ変更)
...
- 実装内容:
  - 新規 [components/FuturePreview.tsx](../components/FuturePreview.tsx):
    ...
    - 左: 「今のカードのサンプル」(白背景 + 黒文字 + シンプル構造)
    - 右: 「未来のカードのサンプル」(紫→ピンクのグラデ背景 + 角丸カード + タグ風リスト + 影)
...
## T22-T27: Lesson 1-6 の段階的ワクワク設計への修正

- 対応要件: §3.5 / §6 Lesson 1-6 / §10.3 / §17.1 / §18.1
- 実装内容:
  - [lib/lessons.ts](../lib/lessons.ts): Lesson 1-5 のプレビュー CSS を最小装飾に統一。背景は `#fafafa`、フォントは `system-ui`、レイアウトは上から自然配置にして、学習者の HTML / CSS / JavaScript が画面変化の主役になるよう変更
  - Lesson 1 文言を「綺麗な画面が作れる」から「自分が書いた文字が画面に出る」へ変更
  - Lesson 4 はプリセット側から見出し色指定を外し、学習者が `color` を書いた瞬間の変化を最大化
  - Lesson 5 は Lesson 4 と同じ最小装飾にそろえ、JS の `textContent` による文字変更だけに集中
  - [components/FuturePreview.tsx](../components/FuturePreview.tsx) / [components/Lesson6Recap.tsx](../components/Lesson6Recap.tsx): 「今のカード」は学習者の実コードではなく固定サンプルであることが伝わる文言へ調整
  - [docs/PROJECT_OVERVIEW.md](PROJECT_OVERVIEW.md) / [docs/PHASE3_REQUIREMENTS.md](PHASE3_REQUIREMENTS.md) / [docs/CURRICULUM.md](CURRICULUM.md): 設計思想を「派手な見た目で感動」から「自分のコードで画面を変える感動」に更新。Lesson 7-16 への申し送りも CURRICULUM に明記
- 要件定義書との差分:
  - **意図的変更あり**: 旧 §10.3 の「1 周目では CSS のお膳立てがあり、コードを書くと部分的にプレビューが変わる」を、「CSS のお膳立ては最小限、学習者のコードによる変化を主役」に更新
  - **実装制約あり**: Lesson 5 で Lesson 4 の学習者コードを永続的に引き継ぐ仕組みは Phase 3.1 にはないため、今回の実装では Lesson 4 と同じ最小装飾の舞台を使う。実コード引き継ぎは Phase 3.3 以降の設計課題
- 自己評価: **要確認**
  - Lesson 1-3 は「地味すぎる」リスクがあるため、Phase 3.1 検証で「続きをやりたい意向」「自分のカードが育っている感」を重点確認する
  - 自動確認は `next build` / matcher 回帰 / Lesson 1-6 ページ確認で実施する
