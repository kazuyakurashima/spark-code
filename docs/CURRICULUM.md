...
Phase 3.1 では、派手な装飾を最初から見せるのではなく、学習者が書いたコードで画面が変わる実感を優先する。
プリセット CSS は読みやすさのための最小限に留め、Lesson 4 以降は学習者自身が書く CSS / JavaScript を変化の主役にする。

| Lesson | 装飾レベル | 変化の主役 |
|---|---|---|
| Lesson 1 | 極薄背景 + 読みやすい system-ui。`<h1>` はブラウザ既定に近い | `<h1>` で名前が表示される |
| Lesson 2 | Lesson 1 と同じ最小装飾 | `<p>` で自己紹介文が増える |
| Lesson 3 | Lesson 1 と同じ最小装飾 | `<ul>` / `<li>` で好きなものリストが増える |
| Lesson 4 | Lesson 3 と同じ最小装飾。初期文字色はブラウザ既定の黒 | 学習者の `color` 指定で見出し色が変わる |
| Lesson 5 | Lesson 4 と同じ最小装飾 | `textContent` で表示文字が変わる |
| Lesson 6 | Recap 画面のみ演出あり | 1 周目の振り返りと、2 周目以降の成長予告 |

Lesson 7〜16 を実装する際も同じ原則を引き継ぐ。学習者が書く CSS / Tailwind / JavaScript が主役であり、お膳立て CSS は段階的な理解を助ける範囲に留める。
...
- **内容**:
  ...
  - Sparkコーチからの総合振り返り
  - 「未来のカード」プレビュー(今のカードのサンプル vs 完成形サンプルを並べて見せる)
...
- **Sparkコーチの総合振り返り例**:
  ...
  > 下の左側は、ここまで作ったカードの今の状態を表すサンプルです。
  > ここから先は、このカードをもっと作品らしく育てていきます。
  > 角丸、影、テーマ色、入力で変わる仕組みまで。
  > 2 周目では、自分のカードを実際にこんな感じに育てていきます。
